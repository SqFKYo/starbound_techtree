...
def parsing_recipes(context):
    context.parser.parse_recipes()


@then("{recipe_name} recipe is found")
def assume_recipe_found(context, recipe_name):
    assert_in(recipe_name, nx.nodes(context.parser.recipes))
...
def assume_recipe_input_output(context, result, materials):
    ...
    for raw in raws:
        assert_in((raw, result), nx.edges(context.parser.recipes))
